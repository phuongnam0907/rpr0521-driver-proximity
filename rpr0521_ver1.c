...
#define RPR0521_THDH1_PS_REG 		0x4B
#define RPR0521_THDH2_PS_REG 		0x4C
#define RPR0521_THDL1_PS_REG 		0x4D
#define RPR0521_THDL2_PS_REG 		0x4E
#define RPR0521_DATA1_OFFSET_REG	0x53
#define RPR0521_DATA2_OFFSET_REG	0x54
...
static ssize_t rpr0521_ps_distance_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	pdata->distanceValue = rpr0521_read_distance(ps_data);
	dev_err(&pdata->client->dev, "RPR0521 -- DEBUG -- val = %d\n", pdata->distanceValue);
...
//     }
// 	w_state_reg = ret;
// 	w_state_reg &= ~(RPR0521_STATE_EN_PS_MASK | RPR0521_STATE_EN_WAIT_MASK | 0x60);
// 	if(enable)
// 	{
// 		w_state_reg |= RPR0521_STATE_EN_PS_MASK;
// 		if(!(ps_data->als_enabled))
// 			w_state_reg |= RPR0521_STATE_EN_WAIT_MASK;
// 	}
...
static int32_t rpr0521_set_ps_thd_l(struct rpr0521_data *ps_data, uint16_t thd_l)
{
    ...
    *(pSrc+1) = temp;
	...
	return i2c_smbus_write_word_data(ps_data->client,RPR0521_THDL1_PS_REG,thd_l);
}
...
static int32_t rpr0521_set_ps_thd_h(struct rpr0521_data *ps_data, uint16_t thd_h)
{
    ...
    *(pSrc+1) = temp;
	...
	return i2c_smbus_write_word_data(ps_data->client,RPR0521_THDH1_PS_REG,thd_h);
}
...
static inline uint32_t rpr0521_get_ps_reading(struct rpr0521_data *ps_data)
{
	...
	tmp_word_data = i2c_smbus_read_word_data(ps_data->client,RPR0521_REG_PXS_DATA);
	if(tmp_word_data < 0)
	{
	...
	// return word_data;
	return rpr0521_read_distance(ps_data);
}
...
{
	struct rpr0521_data *ps_data =  dev_get_drvdata(dev);
    int *val = (int*)kmalloc(sizeof(int), GFP_KERNEL);
	int ret;

	ret = rpr0521_read_ps_offset(ps_data, val);
	if (ret < 0) return ret;
...
static ssize_t rpr0521_ps_offset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = rpr0521_write_ps_offset(ps_data,offset);
	if(ret < 0)
	{
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
