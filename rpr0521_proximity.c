...
static int32_t rpr0521_enable_ps(struct rpr0521_data *ps_data, uint8_t enable)
{
    ...
    if(enable)
	{
		...
		near_far_state = (ret<ps_data->ps_thd_h)?0:1;
...
{
    int32_t word_data, tmp_word_data1, tmp_word_data2;
	...
	word_data = ((tmp_word_data1 & 0xFF00) >> 8) | ((tmp_word_data1 & 0x00FF) << 8);

	tmp_word_data2 = i2c_smbus_read_word_data(ps_data->client, RPR_DATA1_ALS_REG);
	if(tmp_word_data2 < 0)
	{
		printk(KERN_ERR "%s fail, err=0x%x", __func__, tmp_word_data2);
		return tmp_word_data2;
	}
	word_data = ((tmp_word_data2 & 0xFF00) >> 8) | ((tmp_word_data2 & 0x00FF) << 8) ;


	if (ps_data->use_fir)
		word_data = rpr0521_filter_reading(ps_data, word_data);
...
static int32_t rpr0521_get_ir_reading(struct rpr0521_data *ps_data)
{
	...
	w_reg = (uint8_t)(ret & (~RPR_STATE_EN_IRS_MASK));
	w_reg |= RPR_STATE_EN_IRS_MASK;
	...
	tmp_word_data = i2c_smbus_read_word_data(ps_data->client, RPR_DATA1_IR_REG);
	if(tmp_word_data < 0)
	{
		printk(KERN_ERR "%s fail, err=0x%x", __func__, tmp_word_data);
		return tmp_word_data;
...
static struct device_attribute als_enable_attribute = __ATTR(enable,0664,rpr_als_enable_show,rpr_als_enable_store);
...
static struct device_attribute als_transmittance_attribute = __ATTR(transmittance,0664,rpr_als_transmittance_show,rpr_als_transmittance_store);
static struct device_attribute als_poll_delay_attribute = __ATTR(poll_delay, 0664, rpr_als_delay_show, rpr_als_delay_store);
static struct device_attribute als_ir_code_attribute = __ATTR(ircode,0444,rpr_als_ir_code_show,NULL);
static struct device_attribute als_firlen_attribute = __ATTR(firlen, 0664, rpr_als_firlen_show, rpr_als_firlen_store);
static struct device_attribute als_fir_enable_attribute = __ATTR(fir_enable, 0664, rpr_als_fir_enable_show, rpr_als_fir_enable_store);
...
static struct device_attribute ps_enable_attribute = __ATTR(enable,0664,rpr_ps_enable_show,rpr_ps_enable_store);	// DONE
static struct device_attribute ps_distance_attribute = __ATTR(distance,0664,rpr_ps_distance_show, rpr_ps_distance_store);
static struct device_attribute ps_offset_attribute = __ATTR(offset,0664,rpr_ps_offset_show, rpr_ps_offset_store);	// DONE
static struct device_attribute ps_code_attribute = __ATTR(code, 0444, rpr_ps_code_show, NULL);		// DONE
static struct device_attribute ps_code_thd_l_attribute = __ATTR(codethdl,0664,rpr_ps_code_thd_l_show,rpr_ps_code_thd_l_store);	// DONE
static struct device_attribute ps_code_thd_h_attribute = __ATTR(codethdh,0664,rpr_ps_code_thd_h_show,rpr_ps_code_thd_h_store);	// DONE
static struct device_attribute recv_attribute = __ATTR(recv,0664,rpr_recv_show,rpr_recv_store);		// DONE
static struct device_attribute send_attribute = __ATTR(send,0664,rpr_send_show, rpr_send_store);	// DONE
static struct device_attribute all_reg_attribute = __ATTR(allreg, 0444, rpr_all_reg_show, NULL);	// DONE
